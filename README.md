...
## 🛠️ 维护范围
- 持续跟进常用基础服务的主流版本，补充或更新对应的 Dockerfile、`compose.sample.yml` 与 `sample.env`
- 通过 GitHub Actions 对部分镜像构建进行自动化验证，降低配置变更带来的回归风险
- 统一服务目录、环境变量、端口、volume、日志和配置文件组织方式，方便开发者按需组合
- 优先避免将敏感配置提交到版本控制，并在模板中保留可替换、可本地化的示例配置

## 🗺️ Roadmap
- 完善更多服务版本的 CI 构建测试，逐步覆盖现有 Dockerfile 与 Compose 模板
- 增加 Dockerfile、Compose、`.env` 模板的安全基线检查，减少权限、密钥、端口暴露等风险
- 补充各服务的独立 README、升级说明和常见问题，降低新用户使用成本
- 优化多服务组合示例，覆盖 Web、数据库、缓存、消息队列、搜索和证书签发等常见开发场景
